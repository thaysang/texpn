...
export const generateToken = async (req,res,next) => {
    const accessToken = await jwt.sign(req.user,process.env.ACCESS_TOKEN,{expiresIn:"20s"})
...
export const refreshAccessToken = async (req,res,next) => {
    ...
    jwt.verify(req.body.token, process.env.REFRESH_TOKEN, async (err,user) =>{
        ...
        const accessToken = await jwt.sign({username:user.username},process.env.ACCESS_TOKEN,{expiresIn:"20s"})
